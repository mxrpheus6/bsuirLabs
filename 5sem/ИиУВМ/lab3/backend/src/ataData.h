...
#define FEAT_REG 0x1
#define SECTOR_COUNT_REG 0x2

#define LBA_LOW_REG 0x3
#define LBA_MID_REG 0x4
#define LBA_HIG_REG 0x5

#define SELECT_REG 0x6
#define STATUS_REG 0x7
#define COMMAND_REG 0x7

#define FLOATING_BUS 0x7F

#define DEVICE_MASTER 0xA0
#define DEVICE_SLAVE 0xB0

const std::pair<std::string, uint16_t> buses[] = {
    {"Primary Bus", 0x1F0},
    {"Secondary Bus", 0x170},
    {"Extra Primary Bus", 0x3F0},
    {"Extra Secondary Bus", 0x370}
};
...
enum class AtaBus {
    PrimaryBus = 0x1F0,
    SecondaryBus = 0x170,
    ExtraPrimaryBus = 0x3F0,
    ExtraSecondaryBus = 0x370,
};

enum class DeviceOnBusType {
    Master = 0,
    Slave = 1,
    Undefined
};

struct DeviceBaseInformation {
    AtaBus bus;
    DeviceOnBusType position;
    DeviceInterface interf;
    DeviceStatus status;
};

struct PataDevicesInfo {
    struct {
