# Диаграммы последовательностей

# Содержание
1. [Авторизация](#1)
2. [Добавление новой транзакции](#2)
3. [Просмотр истории по месяцу](#3)


### 1. Авторизация<a name="1"></a>

![Авторизация](https://github.com/mxrpheus6/Financik/blob/master/Diagrams/img/authSequence.png)

### 2. Добавление новой транзакции<a name="2"></a>

![Добавление новой транзакции](https://github.com/mxrpheus6/Financik/blob/master/Diagrams/img/addNewSequence.png)
  
### 3. Просмотр истории по месяцу<a name="3"></a>

![Просмотр истории по месяцу](https://github.com/mxrpheus6/Financik/blob/master/Diagrams/img/historySequence.png)