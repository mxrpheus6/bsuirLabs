...
QList<Packet> Packet::receivePackets(QSerialPort *port) {
    ...
    if (port->isOpen() && port->waitForReadyRead(1000)) {
        ...
        const int packetSize = 52;

        for (int i = 0; i < receivedData.size(); i += packetSize) {
            TokenRingDataFrame packet;
            Packet info;

            QByteArray sdBytes = receivedData.mid(i, 1);
            packet.sd = *reinterpret_cast<const uint8_t*>(sdBytes.constData());

            QByteArray acBytes = receivedData.mid(i + 1, 1);
            packet.ac = *reinterpret_cast<const uint8_t*>(acBytes.constData());

            QByteArray fcBytes = receivedData.mid(i + 2, 1);
            packet.fc = *reinterpret_cast<const uint8_t*>(fcBytes.constData());

            QByteArray daBytes = receivedData.mid(i + 3, 8);
            packet.da = *reinterpret_cast<const uint64_t*>(daBytes.constData());

            QByteArray saBytes = receivedData.mid(i + 11, 8);
            packet.sa = *reinterpret_cast<const uint64_t*>(saBytes.constData());

            QByteArray flagBytes = receivedData.mid(i + 19, 8);
            info.flag = *reinterpret_cast<const int64_t*>(flagBytes.constData());

            QByteArray destinationBytes = receivedData.mid(i + 27, 4);
            info.destinationAddress = *reinterpret_cast<const int32_t*>(destinationBytes.constData());

            QByteArray sourceBytes = receivedData.mid(i + 31, 4);
            info.sourceAddress = *reinterpret_cast<const int32_t*>(sourceBytes.constData());

            info.data = receivedData.mid(i + 35, 19);

            QByteArray fcsBytes = receivedData.mid(i + 54, 2);
            info.fcs = *reinterpret_cast<const uint16_t*>(fcsBytes.constData());

            QByteArray fcsFrameBytes = receivedData.mid(i + 56, 1);
            packet.fcs = *reinterpret_cast<const uint8_t*>(fcsFrameBytes.constData());

            QByteArray edBytes = receivedData.mid(i + 57, 1);
            packet.ed = *reinterpret_cast<const uint8_t*>(edBytes.constData());

            QByteArray fsBytes = receivedData.mid(i + 58, 1);
            packet.fs = *reinterpret_cast<const uint8_t*>(fsBytes.constData());

            QByteArray ifgBytes = receivedData.mid(i + 59, 1);
            packet.ifg = *reinterpret_cast<const uint8_t*>(ifgBytes.constData());
