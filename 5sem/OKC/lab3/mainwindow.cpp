...
{
    ui->setupUi(this);
    this->setWindowTitle("Бит-стаффинг");
...
void MainWindow::on_pushButtonSend_1_clicked()
{
    ...
    if (!Packet::transmitPackets(port1, transmittedPackets)) {
    ...
    for (const auto& p : fixedPackets) {
        ui->plainTextEditOutput_2->appendPlainText(p.data);
        ui->plainTextEditOutput_2->appendPlainText("Before  : " + vectorToQString(p.controlBitsBefore));
        ui->plainTextEditOutput_2->appendPlainText("After     : " + vectorToQString(p.controlBitsAfter));
        ui->plainTextEditOutput_2->appendPlainText("Position: " + QString::number(p.errorPosition));
...
void MainWindow::on_pushButtonSend_2_clicked()
{
    ...
    for (const auto& p : fixedPackets) {
        ui->plainTextEditOutput_1->appendPlainText(p.data);
        ui->plainTextEditOutput_1->appendPlainText("Before  : " + vectorToQString(p.controlBitsBefore));
        ui->plainTextEditOutput_1->appendPlainText("After     : " + vectorToQString(p.controlBitsAfter));
        ui->plainTextEditOutput_1->appendPlainText("Position: " + QString::number(p.errorPosition));
