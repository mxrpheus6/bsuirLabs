    ...
    ui(new Ui::RealEstateBase)
{
    ...
    ui->lineEdit_floor_amount_from->setValidator(intValidator);
    ui->lineEdit_floor_amount_to->setValidator(intValidator);
    ui->lineEdit_floor_amount_from->setEnabled(false);
    ui->lineEdit_floor_amount_to->setEnabled(false);

    ui->pushButton_removeDeal->setVisible(false);
    ...
    if (mode == "base") {
        ...
        ui->pushButton_removeDeal->setEnabled(true);
        ui->pushButton_removeDeal->setVisible(true);
...
void RealEstateBase::setFilters() {
    ...
    if (!ui->lineEdit_floor_amount_to->text().isEmpty() || !ui->lineEdit_floor_amount_from->text().isEmpty()) {
        resModel = applyFilterLineEdit(resModel, FLOOR_AMOUNT_COL, ui->lineEdit_floor_amount_from, ui->lineEdit_floor_amount_to);
    }
    ui->tableView_estate->setModel(resModel);
}
...
{
    if (arg1 == Qt::Checked) {
        ui->lineEdit_floor_amount_from->setEnabled(true);
        ui->lineEdit_floor_amount_to->setEnabled(true);
    }
    else {
        ui->lineEdit_floor_amount_from->clear();
        ui->lineEdit_floor_amount_to->clear();
...
void RealEstateBase::on_lineEdit_floor_amount_from_textChanged(const QString &arg1)
{
    Q_UNUSED(arg1);
    setFilters();
}
...
void RealEstateBase::on_lineEdit_floor_amount_to_textChanged(const QString &arg1)
{
    Q_UNUSED(arg1);
    setFilters();
}
...
void RealEstateBase::on_pushButton_delete_clicked()
{
    if (currentRow >= 0) {
        ...
        if (reply == QMessageBox::Yes) {
            ...
            int rowToRemove = findID(tempModel, ID);

            if (rowToRemove != -1) {
                tempModel->removeRow(rowToRemove);
                tempModel->select();
            }
            /*for (int row = 0; row < tempModel->rowCount(); row++) {
                index = tempModel->index(row, ID_COL);
                if (tempModel->data(index).toInt() == ID) {
...
void RealEstateBase::on_pushButton_removeDeal_clicked()
{
    if (currentRow >= 0) {
        QSqlTableModel* tempModel = new QSqlTableModel();
        tempModel->setTable(DEALS);
        ...
        msgBox.setIcon(QMessageBox::Question);
        msgBox.setWindowTitle("Подтверждение отклонения");
        msgBox.setText("Вы уверены, что удалить объект из заключенных сделок?");
        msgBox.addButton(QMessageBox::Yes)->setText("Да");
        msgBox.addButton(QMessageBox::No)->setText("Нет");
        msgBox.setDefaultButton(QMessageBox::No);
        ...
        if (reply == QMessageBox::Yes) {
            ...
            tempModel->select();
            setFilters();
            QMessageBox::information(this, "Успех", "Объект успешно удален из заключенных сделок!");
        }
